...
def prepared_alti(
    ...
    # L3 products
        ...
        if version_swot == "2.0.1":
            dfs["ggde_fromduacsv_unfiltered"] = dfs.f * (
                dfs.duacs_speed_meridional_abs
                - dfs.duacs_speed_meridional
                + dfs.duacs_speed_meridional_unfiltered
            )
            dfs["ggdn_fromduacsv_unfiltered"] = -dfs.f * (
                dfs.duacs_speed_zonal_abs
                - dfs.duacs_speed_zonal
                + dfs.duacs_speed_zonal_unfiltered
            )
        ...
        if alti_diff_method != "fromduacsv":
            ggd_var = [
                v.replace("dx_", "")
                for v in dfs.columns
                if (("etaf" in v) | ("etac" in v)) & ("dx_" in v)
            ]
            ...
            for v in ggd_var:
                ...
                dfs["ggde_" + v] = (
                    g * rotate(dfs["dx_" + v], dfs["dy_" + v], dfs["phi"])[0]
                )
                dfs["ggdn_" + v] = (
                    g * rotate(dfs["dx_" + v], dfs["dy_" + v], dfs["phi"])[1]
                )
                dfs["vorticity_" + v] = (
                    g * (dfs["dxx_" + v] + dfs["dyy_" + v]) / dfs.f**2
                )
                dfs["strain_s_" + v] = (
                    g * (dfs["dxx_" + v] - dfs["dyy_" + v]) / dfs.f**2
                )
            ...
            l = (
                ...
                + ["duacs_relative_vorticity", "duacs_strain"]
            )
...
def prepared_wd(
    ...
    if drifter_preprocess == "spectral_decomp":
        dfw = pd.read_csv(
            os.path.join(
                ...
                "coloc_files",
                "wind",
                "windcoloc_spectral_decomp_12h_all_med_variational_10min_v1.csv",
            ),
        ...
        dfw = pd.read_csv(
            os.path.join(
                ...
                "coloc_files",
                "wind",
                f"{wd_product_key}_{wd_model}_"
                + colocs_source.replace(".nc", "")
                + ".csv",
            )
        ).set_index("row_number")
    ...
    dfw = dfw[
        [v for v in dfw if (spectral_key + "vsde" in v or spectral_key + "vsdn" in v)]
    ]
...
def create_id_comb(
    ...
    if "L4" in alti_product_key:
        ...
        l = ["ggde_" + ggd_var, "ggdn_" + ggd_var, "phi", "distance_to_coast"]
...
def one_comb(
    ...
    if (ggd_var == "fromduacsv") | (ggd_var == "fromduacsv_unfiltered"):
        assert (
            alti_diff_method == "fromduacsv"
        ), "alti_diff_method should be 'fromduacsv' with ggd_var ='fromduacsv' or 'fromduacsv_unfiltered'"
    if alti_diff_method == "fromduacsv":
        assert (ggd_var == "fromduacsv") | (
            ggd_var == "fromduacsv_unfiltered"
        ), "ggd_var should be 'fromduacsv' or 'fromduacsv_unfiltered' with alti_diff_method='fromduacsv'"
