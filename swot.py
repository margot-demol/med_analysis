...
def browse_swot_250m():
    ...
    for p in passes:
        ...
        for f in files:
            ...
            if (p == 3) & (c in [568]):
                continue  # empty cycle_number
            if (p == 16) & (c in [508, 513, 534, 554, 568]):
                continue  # empty cycle_number
            df = xr.open_zarr(f)
            # correct SWOT product time
            if version_swot == "2.0.1":
                df["time"] = df.time - np.timedelta64(946684800000000000, "ns")
...
def add_mask_inside_swot(ds_swot, ds_drifters):
    ...
    left = build_swath_polygon(
        ds_swot, x="longitude", y="latitude", dix=50, diy=50, side="left"
    )
    right = build_swath_polygon(
        ds_swot, x="longitude", y="latitude", dix=50, diy=50, side="right"
    )
    # stack dimensions
    drs = ds_drifters.stack(points=("datetime", "drifter_id"))
    lon, lat = drs.longitude.fillna(0.0).values, drs.latitude.fillna(0.0).values
    # test if points inside right swath
    drs["inside_right"] = (
        "points",
        np.array(
            [right.contains(shp.Point(_lon, _lat)) for _lon, _lat in zip(lon, lat)]
        ),
    )
    ...
    drs["inside_left"] = (
        "points",
        np.array(
            [left.contains(shp.Point(_lon, _lat)) for _lon, _lat in zip(lon, lat)]
        ),
    )
...
def gradient_naive(dss, ggd_variables=None):
    ...
    dss_ggdx = (g * dss_ggd.differentiate("num_pixels") / dx).rename(
        {v: "ggdx_" + v for v in ggd_variables}
    )
    dss_ggdy = (g * dss_ggd.differentiate("num_lines") / dy).rename(
        {v: "ggdy_" + v for v in ggd_variables}
    )

    dss_ggdxx = (dss_ggdx.differentiate("num_pixels") / dx).rename(
        {"ggdx_" + v: "ggdxx_" + v for v in ggd_variables}
    )
    dss_ggdyy = (dss_ggdy.differentiate("num_lines") / dy).rename(
        {"ggdy_" + v: "ggdyy_" + v for v in ggd_variables}
    )
    dss_ggdxy = (dss_ggdx.differentiate("num_lines") / dy).rename(
        {"ggdx_" + v: "ggdxy_" + v for v in ggd_variables}
    )

    return xr.merge([dss_ggdx, dss_ggdy, dss_ggdxx, dss_ggdyy, dss_ggdxy]).where(
...
def strain_vorticity(dsg):
    ...
    dsg["vorticity"] = (
        dsg["ggdxx_duacs_ssha_karin_2_filtered"]
        ...
        + dsg["ggdyy_cvl_ocean_tide_fes_2022"]
    ) / dsg["f"]
    ...
    dsg["sigma_n"] = (
        -2
        * (
            dsg["ggdxy_duacs_ssha_karin_2_filtered"]
            + dsg["ggdxy_cvl_mean_dynamic_topography_cnes_cls_22"]
            + dsg["ggdxy_cvl_ocean_tide_fes_2022"]
        )
        / dsg["f"]
    )
...
def gradient_gauss(dss, cutoff, ggd_variables=None, **kwargs):
    ...
    for v in dss_ggd:
        da = (
            dss_ggd[v].interpolate_na(dim="num_pixels").interpolate_na(dim="num_lines")
        ...
        dss_ggdx["ggdx_" + v] = g * (
            ...
            / dx
        ).where(~dss["duacs_ssha_karin_2_filtered"].isnull())
...
def secondderivative_gauss(dss, cutoff, ggd_variables=None, **kwargs):
    ...
    for v in dss_ggd:
        ...
        dss_ggdx["ggdxx_" + v] = (
            xr.DataArray(
                gaussian_filter(
                    dss_ggdx["ggdx_" + v], sigma=cutoff / dx, order=order, **kwargs
                ),
                dims=da.dims,
            )
            / dx
        ...
        dss_ggdyy["ggdyy_" + v] = (
            xr.DataArray(
                gaussian_filter(
                    dss_ggdy["ggdy_" + v], sigma=cutoff / dy, order=order, **kwargs
                ),
                dims=da.dims,
            )
            / dy
        ...
        dss_ggdxy["ggdxy_" + v] = (
            xr.DataArray(
                gaussian_filter(
                    dss_ggdx["ggdx_" + v], sigma=cutoff / dy, order=order, **kwargs
                ),
                dims=da.dims,
            )
            / dy
...
def etaf(dfs):
    dfs["ggdx_etaf"] = (
        dfs.ggdx_duacs_ssha_karin_2_filtered
        + dfs.ggdx_cvl_mean_dynamic_topography_cnes_cls_22
        + dfs.ggdx_cvl_ocean_tide_fes_2022
    )
    dfs["ggdy_etaf"] = (
        dfs.ggdy_duacs_ssha_karin_2_filtered
        + dfs.ggdy_cvl_mean_dynamic_topography_cnes_cls_22
        + dfs.ggdy_cvl_ocean_tide_fes_2022
    )
...
def coloc_swot_cycle_swath(
    dfr, dfs, cycle, swath, cutoff, ggd_variables, variables, method_gradient="gauss"
):
...
def rotate_ggd(df, ggd_variables):
    for v in ggd_variables:
        df["ggde_" + v], df["ggdn_" + v] = rotate(
            df["ggdx_" + v], df["ggdy_" + v], df["phi"]
        )
