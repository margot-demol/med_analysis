...
if space == "datarmor":
    # DATARMOR
    swot_dir_2km = (
        f"/home/datawork-lops-oc/aponte/swot/cswot/swot/L3_calval_{version_swot}"
    )
...
tminpbswot = [
    pd.to_datetime("2023-04-23 18:40:00"),
...
def remove_pb_swot(df):
    for i in range(len(idpbswot)):
        pb = (
            ...
            & (df.datetime >= tminpbswot[i])
            & (df.datetime <= tmaxpbswot[i])
        )
        df = df[~pb]
    return df
